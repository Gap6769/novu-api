...
from ..models.novel import NovelCreate, NovelPublic, NovelUpdate, PyObjectId, NovelSummary, NovelDetail, NovelType
...
@router.post("/", response_model=NovelPublic, status_code=status.HTTP_201_CREATED, tags=["novels"])
async def create_novel(novel_in: NovelCreate, novel_repository: NovelRepository = Depends(get_novel_repository)):
    ...
    if await novel_repository.exists_by_source_url(str(novel_in.source_url)):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail=f"Novel from source URL {novel_in.source_url} already exists."
        )
...
async def get_novel_by_id(novel_id: PyObjectId, novel_repository: NovelRepository = Depends(get_novel_repository)):
...
async def update_novel(
    novel_id: PyObjectId, novel_update: NovelUpdate, novel_repository: NovelRepository = Depends(get_novel_repository)
):
...
async def delete_novel(novel_id: PyObjectId, novel_repository: NovelRepository = Depends(get_novel_repository)):
...
async def update_metadata(novel_id: PyObjectId, novel_repository: NovelRepository = Depends(get_novel_repository)):
