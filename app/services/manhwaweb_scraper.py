...
class ManhwaWebScraper(BaseScraper):
    ...
    async def get_chapters(self, url: str, max_chapters: int = 50) -> List[Chapter]:
        ...
        async with self:
            ...
            try:
                view_all_button = await self._page.wait_for_selector(
                    self.config.selectors["view_all_button"], timeout=5000
                )
                if view_all_button:
    ...
    async def get_chapter_content(self, url: str, novel_id: str, chapter_number: int) -> Dict[str, Any]:
        ...
        async with self:
            ...
            images = await self._page.query_selector_all(
            ...
            for index, img in enumerate(images):
                ...
                if (
                    src
                    and not any(ad in src for ad in ["pubadx", "ads", "advertisement"])
                    and not src.endswith("loading.gif")
                ):
            ...
            content = {"type": "manhwa", "images": image_list, "total_images": len(image_list)}
